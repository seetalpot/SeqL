...
def hd(ys, k):
    ...
    ws = [zi if bi else ~zi for (bi, zi) in zip(bs,zs)]
    eq = ckt.AndGate(*ws)
    return eq
...
def isonecounter_v1(eq, ps, k):
    ...
    key = {}
    for ((pi, qi), mi, ni) in zip(inps, ms, ns):
        ai, bi = nmap[pi], nmap[qi]
        if mi == ni:
            key[pi] = mi
...
def checkKey(eq, ps, ks, k):
    qs = [~pi if ki else pi for (ki, pi) in zip(ks, ps)]
    eqP = hd(qs, k)
    miter = eq ^ eqP
    S = Solver()
    nmap = {}
...
def test(k, h):
    ...
    ys = [~pi if fi else pi for (fi, pi) in zip(fs, ps)]
    eq  = hd(ys, h)
